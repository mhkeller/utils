...
export default async function drawHistograms(data, { name = '', facetBy = [], fields = [], screenshotOutDir, fill = '#000', css }) {
	...
	for (const fb of facetBy) {
		...
		for (const f of fields) {
			 ...
			 */
			const n = name ? `${name}_` : '';
			const screenshotOutPath = join(screenshotOutDir, `${n}by__${fb}_${f}.png`);
