...
Add the `queueCalls` function. Allow for `logProgress` to have custom messages. Add configurable indent to `logProgress`. Show millieseconds in `prettyMs`. Add `jitter` option to `sleep`.
